/**
 * @requires tent.connectors.rest
 * @name tent.connectors.couchdb
 * @namespace Apache CouchDB RESTful Persister
 */
tent.declare('tent.connectors.couchdb', function(){

    /**
     * Creates a new CouchDB RESTful connector
     * @class a CouchDB RESTful connector
     * @extends tent.connectors.rest.RestConnector
     * @constructor
     */
    tent.connectors.couchdb.CouchDBConnector = function CouchDBConnector(){
        ...
        ctr.filters.load.unshift(function(d){
            if (typeof d.data.rows == 'object' && d.data.rows instanceof Array &&
            typeof d.data.total_rows == 'number') {
                // document set
                d.data.items = [];
        ...
        ctr.filters.saveChanges.push(function(d){
            if (d.data && d.data.items instanceof Array) {
                for (var i = 0, l = d.data.items.length; i < l; i++) {
                    ...
                    if (d.packedChanges && d.packedChanges[i] &&
                    d.packedChanges[i].__changeState__ === tent.entities.ChangeStates.DELETED) {
                        doc._deleted = true;
                    }
                }
        ...
        ctr.filters.saveResult.unshift(function(d){
            for (var i = 0, l = d.data.items.length; i < l; i++) {
                ...
                if (!doc.error && d.packedChanges && d.packedChanges[i] &&
                d.packedChanges[i].__changeState__ === tent.entities.ChangeStates.DELETED) {
                    doc._deleted = true;
                }
            }
