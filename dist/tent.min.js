...
function(){if(typeof this._isDomObject=="undefined")this._isDOMObject=tent.isDOMObject(this.subject);return this._isDOMObject};tent.changes.Observable.prototype.interceptArrayModifiers=function(){if(!this.subject instanceof Array)return this;var d=this.subject;this.subject.setLength=function(e){if(this.length>e)this.splice(e,this.length-e);else this.lengh=e};this.interceptFunction("push",function(){var e=this.length,g=Array.prototype.slice.call(arguments);this.__observable__.notifyChange(tent.changes.EventTypes.ADDING,
...
this.interceptFunction("unshift",function(){var e=Array.prototype.slice.call(arguments);this.__observable__.notifyChange(tent.changes.EventTypes.ADDING,{items:e,index:0,propertyName:this.__propertyName__});var g=this.__observable__.shouldNotifyParent();g&&this.__observable__.notifyParentChanging({type:tent.changes.EventTypes.ADDING,data:{items:e,index:0,propertyName:this.__propertyName__}});this._unshift.apply(this,arguments);this.__observable__.notifyChange(tent.changes.EventTypes.ADDED,{items:e,
...
tent.entities.ContextChangeHandler.prototype.objectLinked=function(a,b,c,f){if(typeof f=="object")if(f instanceof Array){if(f.__parent__!=a){if(f.__parent__)throw"this array already has another parent object";f.__parent__=a}if(f.__propertyName__!=b)f.__propertyName__=b;tent.changes.bind(f,this);c.onLinkPush&&this.context.attach.push(this.context,f)}else if(f!==null&&typeof f!="undefined"){if(f.__changeState__==tent.entities.ChangeStates.DELETED)throw'cannot link a DELETED object to property "'+c+
'"';if(f.__collection__){if(f.__collection__.context!=this.context)throw"cannot link this object, it belongs to another Context";if(c.collection&&f.__collection__.name!=c.collection)throw"cannot link an object of collection "+c.collection+' to property "'+c+'"';}if(!f.__collection__&&c.onLinkPush){f.__entityType__&&this.context.push(f.__entityType__);if(c.collection){if(!this.context.__collections__[c.collection])throw Error('Collection "'+c.collection+'" not found in this Context');this.context.__collections__[c.collection].push(f)}else this.context.push(f)}if(c.trackLinks){c=
null;for(var h=this.context.changes.items.length-1;h>=0;h--){var j=this.context.changes.items[h];if(j instanceof tent.entities.EntityLink&&j.from===a&&j.to===f&&j.propertyName==b){c=j;break}}if(c){if(c.__changeState__==tent.entities.ChangeStates.DELETED)c.__changeState__=tent.entities.ChangeStates.UNCHANGED}else{c=new tent.entities.EntityLink(a,f,b,tent.entities.ChangeStates.ADDED);this.context.changes.items.push(c)}}}};tent.entities.ContextChangeHandler.prototype.objectUnlinked=function(a,b,c,f){if(typeof f==
"object")if(f instanceof Array){delete f.__parent__;delete f.__propertyName__;tent.changes.unbind(f,this);c.onUnlinkRemove&&this.context.remove.apply(this.context,f)}else if(f!==null&&typeof f!="undefined"){c.onUnlinkRemove&&f.__collection__&&f.__collection__.remove(f);if(c.trackLinks){c=null;for(var h=-1,j=this.context.changes.items.length-1;j>=0;j--){var d=this.context.changes.items[j];if(d instanceof tent.entities.EntityLink&&d.from===a&&d.to===f&&d.propertyName==b){c=d;h=j;break}}if(c)if(c.__changeState__==
tent.entities.ChangeStates.ADDED)this.context.changes.items.splice(h,1);else{if(c.__changeState__!=tent.entities.ChangeStates.DELETED)c.__changeState__=tent.entities.ChangeStates.DELETED}else{c=new tent.entities.EntityLink(a,f,b,tent.entities.ChangeStates.DELETED);this.context.changes.items.push(c)}}}};tent.entities.ContextChangeHandler.prototype.getTrackedProperty=function(a,b){var c;if(a.__entityType__&&a.__changeState__&&a.__changeState__!=tent.entities.ChangeStates.DELETED&&a.__changeState__!=
...
b,a.data.current)}}else this.context.touch(a.subject);else if(a.eventType==tent.changes.EventTypes.ADDED){var c=a.subject.__parent__;if(c&&(b=this.getTrackedProperty(c,a.data.propertyName)))for(var f=0,h=a.data.items.length;f<h;f++)this.objectLinked(c,a.data.propertyName,b,a.data.items[f])}else if(a.eventType==tent.changes.EventTypes.REMOVED)if((c=a.subject.__parent__)&&(b=this.getTrackedProperty(c,a.data.propertyName))){f=0;for(h=a.data.items.length;f<h;f++)this.objectUnlinked(c,a.data.propertyName,
b,a.data.items[f])}tent.changes.reverseProperties.getHandler()(a)}};tent.entities.ContextChangeHandler.prototype.init=function(){if(this.context.log)var a=new tent.coreTypes.NameCounter;for(var b in this.context.__collections__){for(var c=this.context.__collections__[b],f=0,h=c.items.length;f<h;f++)this.itemAttached(c.items[f]);a&&a.add(b,c.items.length)}if(this.context.log)a&&a.root._count>0?tent.log.debug("Context: initialized with "+a):tent.log.debug("Context: initialized empty")};tent.entities.ContextChangeHandler.prototype.acceptAllChanges=
function(){if(this.context.hasChanges()){this.context.log&&tent.log.debug("Context: accepting changes, "+this.context.changes);for(var a=this.context.changes.items,b=0,c=a.length;b<c;b++)a[b].__changeState__=tent.entities.ChangeStates.UNCHANGED;a.length=0;this.context.log&&tent.log.debug("Context: all changes accepted")}};tent.entities.ContextChangeHandler.prototype.acceptChanges=function(){for(var a=0,b=arguments.length;a<b;a++){var c=arguments[a],f;if(typeof c=="number"){f=c;if(f<0||this.context.changes.items.length<
f-1){c=null;f=-1}else c=this.context.changes.items[f]}else f=this.context.changes.items.lastIndexOf(c);if(f>=0&&c){if(c.__changeState__===tent.entities.ChangeStates.ADDED||c.__changeState__===tent.entities.ChangeStates.MODIFIED){this.context.changes.items.splice(f,1);c.__changeState__=tent.entities.ChangeStates.UNCHANGED;if(c.__syncState__&&c.__syncState__.changedWhileSaving){delete c.__syncState__;this.context.touch(c)}}else if(c.__changeState__===tent.entities.ChangeStates.DELETED)if(c.__syncState__&&
c.__syncState__.deletedWhileSaving)delete c.__syncState__;else{this.context.changes.items.splice(f,1);c.__changeState__=tent.entities.ChangeStates.DETACHED}delete c.__syncState__}}}});tent.declare("tent.persisters.rest",function(){tent.persisters.rest.uriCombine=function(){for(var a="",b=0,c=arguments.length;b<c;b++){var f=arguments[b]+"";if(f)if(f.match(/^[A-Za-z]+\:\/\//))a=f;else a+=a.substr(a.length-1,1)=="/"?f.substr(0,1)=="/"?f.substr(1):f.substr(0,2)=="./"?f.substr(2):f:f.substr(0,1)=="/"?f:f.substr(0,2)=="./"?f.substr(1):"/"+f}return a};tent.persisters.rest.createItemMapper=function(a){a||(a={});return function(b,c,f){if(b=b instanceof Array?b:c.multi?(c.multiSelector||
...
function(a,b,c){if(typeof jQuery=="undefined"||typeof jQuery.ajax=="undefined")throw"jQuery.ajax is required in order to persist changes";var f,h,j,d;if(b.bulk){f=b.uri||this.bulkSaveUri;if(!f)throw"no bulk save uri provided";f=tent.persisters.rest.uriCombine(this.baseUri,f);j=this.__getPersistData__(a,b);a.markAsSaving(a.changes.items)}else{d=a.changes.items[b.offset||0];a.markAsSaving(d);h=this.getChangeItemMethod(d);if(d=this.getChangeItemUri(d)){f=tent.persisters.rest.uriCombine(this.baseUri,
d);j=this.__getPersistData__(a,b)}}if(j==null){a={persister:this,options:b,ok:true,nochanges:true};b.complete&&b.complete(a)}else jQuery.ajax({context:{persister:this,context:a,options:b,prevCount:c},url:f,beforeSend:function(e){if(b.credentials){e.setRequestHeader("Origin",document.location.protocol+"//"+document.location.host);e.setRequestHeader("Authorization","Basic "+b.credentials)}},type:h||b.method||"POST",cache:false,data:JSON.stringify(j),dataType:"json",contentType:"application/json",success:function(e){var g=
{persister:this.persister,options:this.options};try{this.persister.__processPersistResponse__(this.context,e,b,g);if(this.prevCount)g.count+=this.prevCount;g.ok=true}catch(i){g.error=i}b.complete&&b.complete(g)},error:function(e,g,i){e={persister:this.persister,options:this.options,error:{type:g,errorThrown:i}};if(this.prevCount)e.count+=this.prevCount;b.complete&&b.complete(e)}})};tent.persisters.rest.RestPersister.prototype.__getPersistData__=function(a,b){if(!this.changeSerializer)this.changeSerializer=
tent.persisters.rest.createChangeSerializer();var c=null;if(a.hasChanges())c=this.changeSerializer(a.changes.items,b);return c};tent.persisters.rest.RestPersister.prototype.__processPersistResponse__=function(a,b,c,f){if(a.hasChanges()){if(!this.changeResponseMapper)this.changeResponseMapper=tent.persisters.rest.createChangeResponseMapper();var h=this;this.changeResponseMapper(b,c,function(j,d){var e=a.changes.items[d];if(j.error){if(!e.__saveErrors__)e.__saveErrors__=[];e.__saveErrors__.push({error:j.error,
reason:j.reason})}else{h.updateLocalVersion(e,j);e.__saveErrors__&&delete e.__saveErrors__;a.acceptChanges(d);f.count=(f.count||0)+1}})}}});tent.declare("tent.persisters.couchdb",function(){tent.persisters.couchdb.CouchDBPersister=function(){tent.persisters.rest.RestPersister.apply(this,arguments);this.bulkSaveUri="_bulk_docs";this.loadItemMapper=tent.persisters.rest.createItemMapper({multiSelectorProperty:"rows",multiItemSelectorProperty:"doc"})};tent.mixin(tent.persisters.couchdb.CouchDBPersister,tent.persisters.rest.RestPersister)});
