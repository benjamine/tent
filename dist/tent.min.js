...
tent.entities.ChangeStates.UNCHANGED;else if(c.__changeState__==tent.entities.ChangeStates.MODIFIED)c.__changeState__=tent.entities.ChangeStates.UNCHANGED;else if(c.__changeState__==tent.entities.ChangeStates.DELETED)c.__changeState__=tent.entities.ChangeStates.DETACHED}}}});tent.declare("tent.persisters.rest",function(){tent.persisters.rest.uriCombine=function(){for(var a="",b=0,c=arguments.length;b<c;b++){var e=arguments+"";if(e)if(e.match(/^[A-Za-z]+\:\/\//))a=e;else a+=a.substr(a.length-1,1)=="/"?e.substr(0,1)=="/"?e.substr(1):e.substr(0,2)=="./"?e.substr(2):e:e.substr(0,1)=="/"?e:e.substr(0,2)=="./"?e.substr(1):"/"+e}};tent.persisters.rest.RestPersister=function(){this.baseUri="http://127.0.0.1:5984/mydb";this.saving=false;this.savingErrors=[];this.loadingErrors=
[];this.loadItemMapper=null;this.localItemFinder=function(a,b){return a.filter(function(c){if(typeof b._id!="undefined"&&c._id===b._id||typeof b.id!="undefined"&&c.id===b.id)return true;return false})[0]};this.versionEquals=function(a,b){return a._rev===b._rev};this.updateLocalVersion=function(a,b){return a._rev=b.rev||b._rev};this.isDeleted=function(a){return a._deleted};this.bulkSaveUri=this.changeResponseMapper=this.changeSerializer=null};tent.persisters.rest.RestPersister.prototype.load=function(a,
b,c){try{if(!a)throw"a context is required for loading entities";if(!b)throw"an url must be specified to load";if(this.loading)throw"already loading entities";this.loading=true;var e=this;c||(c={});var g=c.complete;c.complete=function(d){e.loading=false;d.error&&e.loadingErrors.push(d.error);g&&g(d)};this.__load__(a,b,c)}catch(j){this.loadingErrors.push(j);this.loading=false;callback&&callback({error:j})}};tent.persisters.rest.createItemMapper=function(a){a||(a={});return function(b,c,e){if(b=b instanceof
Array?b:c.multi?(c.multiSelector||a.multiSelector||function(h,i){var k=i.multiSelectorProperty||a.multiSelectorProperty;if(k)return h[k];else for(var l in h)if(h[l]instanceof Array)return h[l]})(b,c):(c.singleSelector||a.singleSelector||function(h,i){var k=i.singleSelectorProperty||a.singleSelectorProperty;return k?[h[k]]:[h]})(b,c))for(var g=0,j=b.length;g<j;g++){var d=b[g];if(c.multi)d=(c.multiItemSelector||a.multiItemSelector||function(h,i){var k=i.multiItemSelectorProperty||a.multiItemSelectorProperty;
return k?h[k]:h})(d,c);if(d){var f=c.itemTransformer||a.itemTransformer;if(f)d=f(d,c)}d&&e(d,c)}}};tent.persisters.rest.createChangeSerializer=function(a){a||(a={});return function(b,c){var e=c.itemSerializer||a.itemSerializer||function(h){if(h instanceof tent.entities.EntityLink)return null;for(var i in obj)if(obj.hasOwnProperty(i)&&(i.substr(0,1)!=="_"||i==="_id"||i==="_rev"))tent.pget(obj,i)};if(b instanceof Array){for(var g=[],j=0,d=b.length;j<d;j++)g.push(e(b[j],c));var f=c.saveWrapperProperty||
a.saveWrapperProperty||"docs";return(c.itemsSaveWrapper||a.itemsSaveWrapper||function(h){var i={};i[f]=h;return i})(g,c)}else if(typeof b=="object")return e(b,c)}};tent.persisters.rest.createChangeResponseMapper=function(a){a||(a={});return function(b,c,e){var g;if(g=b instanceof Array?b:(c.resultSelector||a.resultSelector||function(h,i){var k=i.resultSelectorProperty||a.resultSelectorProperty;if(k)return h[k];else{g=h;for(var l in h)if(h[l]instanceof Array)return h[l]}})(b,c)){b=0;for(var j=g.length;b<
j;b++){var d=g[b];if(d=(c.resultItemSelector||a.resultItemSelector||function(h,i){var k=i.resultItemSelectorProperty||a.resultItemSelectorProperty;return k?h[k]:h})(d,c)){var f=c.resultItemTransformer||a.resultItemTransformer;if(f)d=f(d,c)}if(d){f=(c.localItemFinder||a.localItemFinder||function(h,i){if(h.hasChanges())for(var k=0,l=h.changes.items.length;k<l;k++){if(typeof i.id!="undefined"&&h.changes.items[k]._id===i.id)return k;if(typeof i.id!="undefined"&&h.changes.items[k].id===i.id)return k;if(typeof i._id!=
"undefined"&&h.changes.items[k]._id===i._id)return k;if(typeof i._id!="undefined"&&h.changes.items[k].id===i._id)return k}})(c.context,d,c);typeof f=="number"&&f>=0&&e(d,f,c)}}}}};tent.persisters.rest.RestPersister.prototype.__load__=function(a,b,c){if(typeof jQuery=="undefined"||typeof jQuery.ajax=="undefined")throw"jQuery.ajax is required in order to load entities";b=tent.persisters.rest.uriCombine(this.baseUri,b);jQuery.ajax({context:{persister:this,context:a,options:c},url:b,beforeSend:function(e){if(c.credentials){e.setRequestHeader("Origin",
document.location.protocol+"//"+document.location.host);e.setRequestHeader("Authorization","Basic "+c.credentials)}},type:c.method||"GET",cache:!!c.cache,dataType:"json",success:function(e){var g={persister:this,options:c};try{this.persister.__processLoadResponse__(this.context,e,c,g);g.ok=true}catch(j){g.error=j}c.complete&&c.complete(g)},error:function(e,g,j){e={persister:this,options:c,error:{type:g,errorThrown:j}};c.complete&&c.complete(e)}})};tent.persisters.rest.RestPersister.prototype.__processLoadResponse__=
function(a,b,c){if(c.context!==a)c.context=a;if(!this.loadItemMapper)this.loadItemMapper=tent.persisters.rest.createItemMapper();this.loadItemMapper(b,c,function(e){this.localItemFinder(a,e._id);if(obj)if(obj.__changeState__===tent.entities.ChangeStates.MODIFIED||obj.__changeState__===tent.entities.ChangeStates.DELETED)this.versionEquals(obj,e)||obj.__loadErrors__.push({error:"conflict",reason:"document modified recently by another user"});else{if(this.isDeleted(e)){a.remove(obj);a.detach(obj)}else tent.pset(obj,
e,true);obj.__loadErrors__&&delete obj.__loadErrors__}else a.attach(e)})};tent.persisters.rest.RestPersister.prototype.saveChanges=function(a,b){try{if(!a)throw"a context is required for saving changes";b||(b={});if(!b.context)b.context=a;if(a.hasChanges()){if(this.saving)throw"Already saving changes";this.saving=true;var c=this,e=b.complete;b.complete=function(j){c.saving=false;if(j.error)c.savingErrors.push(j.error);else if(typeof j.ok=="undefined")j.ok=true;e&&e(j)};this.__persist__(a,b)}else b.complete&&
b.complete({ok:true,nochanges:true})}catch(g){this.savingErrors.push(g);this.saving=false;b.complete&&b.complete({error:g})}};tent.persisters.rest.RestPersister.prototype.__persist__=function(a,b){if(typeof jQuery=="undefined"||typeof jQuery.ajax=="undefined")throw"jQuery.ajax is required in order to persist changes";if(this.bulkSaveUri){var c=tent.persisters.rest.uriCombine(this.baseUri,this.bulkSaveUri),e=this.__getPersistData__(a,b);if(e==null){c={persister:this,ok:true,nochanges:true};b.complete&&
b.complete(c)}else jQuery.ajax({context:{persister:this,context:a},url:c,beforeSend:function(g){if(b.credentials){g.setRequestHeader("Origin",document.location.protocol+"//"+document.location.host);g.setRequestHeader("Authorization","Basic "+b.credentials)}},type:"POST",cache:false,data:JSON.stringify(e),dataType:"json",contentType:"application/json",success:function(g){var j={persister:this};try{this.persister.__processPersistResponse__(this.context,g,b,j);j.ok=true}catch(d){j.error=d}b.complete&&
b.complete(j)},error:function(g,j,d){g={persister:this,error:{type:j,errorThrown:d}};b.complete&&b.complete(g)}})}else throw"individual change save not implemented";};tent.persisters.rest.RestPersister.prototype.__getPersistData__=function(a,b){if(!this.changeSerializer)this.changeSerializer=tent.persisters.rest.createChangeSerializer();var c=null;if(a.hasChanges())c=this.changeSerializer(a.changes.items,b);return c};tent.persisters.rest.RestPersister.prototype.__processPersistResponse__=function(a,
b,c){if(a.hasChanges()){if(!this.changeResponseMapper)this.changeResponseMapper=tent.persisters.rest.createChangeResponseMapper();this.changeResponseMapper(b,c,function(e,g){var j=a.changes.items[g];if(e.error){if(!j.__saveErrors__)j.__saveErrors__=[];j.__saveErrors__.push({error:e.error,reason:e.reason})}else{this.updateLocalVersion(j,e);j.__saveErrors__&&delete j.__saveErrors__;a.acceptChanges(g)}})}}});
