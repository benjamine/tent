...
e,h){c[e]=h;c.__values__[e]=h;if(typeof c.__names__[h]=="undefined")c.__names__[h]=e;if(typeof c.__maxValue__=="undefined"||c.__maxValue__<h)c.__maxValue__=h};tent.coreTypes.Enum.prototype.add=function(){if(arguments.length<1)return this;for(var c,e,h=0,j=arguments.length;h<j;h++){c=arguments[h];if(c instanceof Array)this.add.apply(this,c);else if(typeof c=="object")for(e in c)if(c.hasOwnProperty(e)){var d=this.__values__[e];if(!d instanceof Number||typeof this.__names__[d]!="undefined")d=this.__freeValue__();
a(this,e,d)}if(typeof c=="string")if(c){c=c.split(",");d=0;for(var f=c.length;d<f;d++){e=c[d].replace(/^\s+/g,"").replace(/\s+$/g,"");if(e=="_FLAGS")this.useFlags(true);else{if(typeof this.__values__[e]!="undefined")if(this.__values__[e]instanceof Number)throw"Enum duplicate name found: "+e;else throw"Enum invalid name (reserved): "+e;if(d==f-1&&h<j-1&&typeof arguments[h+1]=="number"){a(this,e,arguments[h+1]);h++}else a(this,e,this.__freeValue__())}}}}return this};var b=function(){var c=this._name+
(this._count>1?"("+this._count+")":"");if(this._childrenCount){c+="{";var e=0,h;for(h in this)if(h.substr(0,1)!="_"&&typeof this[h]=="object"){if(e>0)c+=", ";c+=this[h];e++}c+="}"}return c};tent.coreTypes.NameCounter=function(c){this.root={_name:c||"",_count:0,toString:b}};tent.coreTypes.NameCounter.prototype.__nodeAdd__=function(c,e){c._count=(c._count||0)+e;if(c._parent){if(c._count==0&&!c._childrenCount){c._parent._childrenCount&&c._parent._childrenCount--;delete c._parent[c._name]}this.__nodeAdd__(c._parent,
e)}};tent.coreTypes.NameCounter.prototype.add=function(c,e){if(typeof e!="number")e=1;for(var h=c.split("."),j=this.root,d=0;d<h.length;d++){if(typeof j[h[d]]=="undefined"){j._childrenCount=(j._childrenCount||0)+1;j[h[d]]={_parent:j,_name:h[d],_count:0,toString:b}}j=j[h[d]]}this.__nodeAdd__(j,e)};tent.coreTypes.NameCounter.prototype.reset=function(){this.root={_name:this.root._name,_count:0,toString:b}};tent.coreTypes.NameCounter.prototype.toString=function(){return b.apply(this.root)}});tent.declare("tent.logging",function(){tent.logging.Levels=new tent.coreTypes.Enum("TRACE,DEBUG,INFO,WARN,ERROR,FATAL");tent.logging.Log=function(){this.listeners=[]};tent.logging.Log.prototype.bindToConsole=function(a){typeof console!="undefined"&&this.bind(tent.logging.consoleLogger,a);return this};tent.logging.Log.prototype.bindToAlert=function(a){this.bind(tent.logging.alertLogger,a);return this};tent.logging.Log.prototype.bindToHtml=function(a,b){this.bind(tent.logging.createHtmlAppendLogger(a),
...
tent.changes.Change=function(d,f,g){this.subject=d;this.eventType=f;this.data=g};tent.changes.ChangeStringifiers=[];tent.changes.Change.prototype.toString=function(){if(typeof tent.changes.ChangeStringifiers[this.eventType]!="undefined")tent.changes.ChangeStringifiers[this.eventType](this);else{var d=tent.changes.EventTypes.getName(this.eventType);if(this.eventType==tent.changes.EventTypes.MANYCHANGES){d+=": ";if(this.data.length<=3)for(var f=0,g=this.data.length;f<g;f++)d+=this.data[f].toString()+
",";else{var i={};f=0;for(g=this.data.length;f<g;f++)i[this.data[f].eventType]=(i[this.data[f].eventType]||0)+1;for(var k in i)d+=tent.changes.EventTypes.getName(k)+"("+i[k]+"),"}}else{if(this.data.propertyName){d+=" "+this.data.propertyName;if(this.eventType==tent.changes.EventTypes.CHANGING)d+=" from "+this.data.current+" to "+this.data.newValue;else if(this.eventType==tent.changes.EventTypes.CHANGED)d+=" from "+this.data.oldValue+" to "+this.data.current;d+=" (on "+this.subject+")"}if(this.data.items){f=
this.data.items;d+=" "+f.length+(f.length>1?" items":" item");if(typeof this.data.index!="undefined")d+=" at index "+this.data.index;d+=this.data.propertyName?" ("+this.subject.__parent__+"."+this.data.propertyName+")":" (["+f+"])"}}return"[Change: "+d+"]"}};tent.changes.Observable=function(d){this.subject=d;this.handlers={};this.interceptors={};this.suspended=false;this.changesWhileSuspended=null};tent.changes.Observable.prototype.interceptFunction=function(d,f){if(!this.interceptors[d]){var g="_"+
d,i={name:d,_name:g,type:tent.changes.InterceptorTypes.FUNCTION};this.subject[g]=this.subject[d];this.subject[d]=f;this.interceptors[d]=i}};tent.pset=function(d,f,g,i){if(typeof f=="object"){for(var k in f)f.hasOwnProperty(k)&&tent.pset(d,k,f[k],g);return d}else{if(d.__observable__&&d.__observable__.interceptors&&d.__observable__.interceptors[f])if(i)d[d.__observable__.interceptors[f]._name||f]=g;else d.__observable__.interceptors[f].newsetter.call(d,g);else d[f]=g;return g}};tent.pget=function(d,
f){return d.__observable__&&d.__observable__.interceptors&&d.__observable__.interceptors[f]?d.__observable__.interceptors[f].newgetter.call(d):d[f]};tent.changes.Observable.prototype.interceptProperty=function(d,f,g,i){if(this.interceptors[d])return this.interceptors[d];var k={name:d,type:tent.changes.InterceptorTypes.PROPERTY,newsetter:i,newgetter:g};if(f){f="_"+d;try{this.subject[f]=this.subject[d]}catch(l){this.subject[f]=null}k._name=f}f=tent.changes.getPropertyInterceptMode();if(f==tent.changes.PropertyInterceptModes.DEFINEPROPERTY||
f==tent.changes.PropertyInterceptModes.DEFINEPROPERTYONLYDOM&&tent.isDOMObject(this.subject)){try{k.descriptor=Object.getOwnPropertyDescriptor(this.subject,d)}catch(m){}f={};if(g)f.get=g;if(i)f.set=i;try{Object.defineProperty(this.subject,d,f)}catch(n){tent.log.warn("Object.defineProperty for property '"+d+"' failed: "+n)}}else if(f==tent.changes.PropertyInterceptModes.DEFINESETTER){try{k.getter=this.subject.__lookupGetter__(d);k.setter=this.subject.__lookupSetter__(d)}catch(p){}if(g)try{this.subject.__defineGetter__(d,
...
tent.changes.Observable.prototype.notifyParentChanging=function(d){d.subject=this.subject;this.parentObject.__observable__.notifyChange(tent.changes.EventTypes.CHANGING,{propertyName:this.parentObjectPropertyName,innerChange:d})};tent.changes.Observable.prototype.notifyParentChanged=function(d){d.subject=this.subject;this.parentObject.__observable__.notifyChange(tent.changes.EventTypes.CHANGED,{propertyName:this.parentObjectPropertyName,innerChange:d})};var b=function(d,f){return function(g){var i=
this[f];if(i!=g){this.__observable__.notifyChange(tent.changes.EventTypes.CHANGING,{propertyName:d,current:i,newValue:g});var k=this.__observable__.shouldNotifyParent();k&&this.__observable__.notifyParentChanging({propertyName:d,current:i,newValue:g});this[f]=g;this.__observable__.notifyChange(tent.changes.EventTypes.CHANGED,{propertyName:d,current:g,oldValue:i});k&&this.__observable__.notifyParentChanged({propertyName:d,current:g,oldValue:i})}}},c=function(d,f){return function(){return this[f]}},
e=function(d,f){return f.substr(0,1)!="_"};tent.changes.Observable.prototype.interceptProperties=function(d){d||(d={});var f=d.propertyFilter||e,g=d.backPropertyPrefix||"_";!d.trackDomProperties&&this.isDOMObject();for(var i in this.subject)if(!tent.isDOMProperty(i))try{if(typeof this.subject[i]!="function"&&f(this.subject,i)){d=g+i;this.interceptProperty(i,d,c(i,d),b(i,d))}}catch(k){tent.log.warn("Error intercepting property '"+i+"': "+k)}return this};tent.changes.Observable.prototype.isDOMObject=
function(){if(typeof this._isDomObject=="undefined")this._isDOMObject=tent.isDOMObject(this.subject);return this._isDOMObject};tent.changes.Observable.prototype.interceptArrayModifiers=function(){if(!this.subject instanceof Array)return this;var d=this.subject;this.subject.setLength=function(f){if(this.length>f)this.splice(f,this.length-f);else this.lengh=f};this.interceptFunction("push",function(){var f=this.length,g=g=Array.prototype.slice.call(arguments);this.__observable__.notifyChange(tent.changes.EventTypes.ADDING,
{items:g,index:f,propertyName:this.__propertyName__});var i=this.__observable__.shouldNotifyParent();i&&this.__observable__.notifyParentChanging({type:tent.changes.EventTypes.ADDING,data:{items:g,index:f,propertyName:this.__propertyName__}});this._push.apply(this,arguments);this.__observable__.notifyChange(tent.changes.EventTypes.ADDED,{items:g,index:f,propertyName:this.__propertyName__});i&&this.__observable__.notifyParentChanged({type:tent.changes.EventTypes.ADDED,data:{items:g,index:f,propertyName:this.__propertyName__}})});
this.interceptFunction("unshift",function(){var f=f=Array.prototype.slice.call(arguments);this.__observable__.notifyChange(tent.changes.EventTypes.ADDING,{items:f,index:0,propertyName:this.__propertyName__});var g=this.__observable__.shouldNotifyParent();g&&this.__observable__.notifyParentChanging({type:tent.changes.EventTypes.ADDING,data:{items:f,index:0,propertyName:this.__propertyName__}});this._unshift.apply(this,arguments);this.__observable__.notifyChange(tent.changes.EventTypes.ADDED,{items:f,
index:0,propertyName:this.__propertyName__});g&&this.__observable__.notifyParentChanged({type:tent.changes.EventTypes.ADDED,data:{items:f,index:0,propertyName:this.__propertyName__}})});this.interceptFunction("pop",function(){var f=this.length-1;this.__observable__.notifyChange(tent.changes.EventTypes.REMOVING,{items:[this[f]],index:f,propertyName:this.__propertyName__});var g=this.__observable__.shouldNotifyParent();g&&this.__observable__.notifyParentChanging({type:tent.changes.EventTypes.REMOVING,
data:{items:[this[f]],index:f,propertyName:this.__propertyName__}});var i=this._pop();this.__observable__.notifyChange(tent.changes.EventTypes.REMOVED,{items:[i],index:f,propertyName:this.__propertyName__});g&&this.__observable__.notifyParentChanged({type:tent.changes.EventTypes.REMOVED,data:{items:[i],index:f,propertyName:this.__propertyName__}});return i});this.interceptFunction("shift",function(){this.__observable__.notifyChange(tent.changes.EventTypes.REMOVING,{items:[this[0]],index:0,propertyName:this.__propertyName__});
var f=this.__observable__.shouldNotifyParent();f&&this.__observable__.notifyParentChanging({type:tent.changes.EventTypes.REMOVING,data:{items:[this[0]],index:0,propertyName:this.__propertyName__}});var g=this._shift();this.__observable__.notifyChange(tent.changes.EventTypes.REMOVED,{items:[g],index:0,propertyName:this.__propertyName__});f&&this.__observable__.notifyParentChanged({type:tent.changes.EventTypes.REMOVED,data:{items:[g],index:0,propertyName:this.__propertyName__}});return g});this.interceptFunction("splice",
function(f,g){var i,k=this.__observable__.shouldNotifyParent();if(g&&g>0){this.__observable__.notifyChange(tent.changes.EventTypes.REMOVING,{items:this.slice(f,f+g),index:f,propertyName:this.__propertyName__});k&&this.__observable__.notifyParentChanging({type:tent.changes.EventTypes.REMOVING,data:{items:this.slice(f,f+g),index:f,propertyName:this.__propertyName__}})}if(arguments.length>2){i=Array.prototype.slice.call(arguments,2);this.__observable__.notifyChange(tent.changes.EventTypes.ADDING,{items:i,
index:f,propertyName:this.__propertyName__});k&&this.__observable__.notifyParentChanging({type:tent.changes.EventTypes.ADDING,data:{items:i,index:f,propertyName:this.__propertyName__}})}if(i&&i.length>0){var l=i.slice(0);l.unshift(f,g);l=this._splice.apply(this,l)}else l=this._splice(f,g);if(l&&l.length>0){this.__observable__.notifyChange(tent.changes.EventTypes.REMOVED,{items:l,index:f,propertyName:this.__propertyName__});k&&this.__observable__.notifyParentChanged({type:tent.changes.EventTypes.REMOVED,
data:{items:l,index:f,propertyName:this.__propertyName__}})}if(arguments.length>2){this.__observable__.notifyChange(tent.changes.EventTypes.ADDED,{items:i,index:f,propertyName:this.__propertyName__});k&&this.__observable__.notifyParentChanged({type:tent.changes.EventTypes.ADDED,data:{items:i,index:f,propertyName:this.__propertyName__}})}return l});d.remove=tent.arrays.functions.remove;d.set=tent.arrays.functions.set;if(!d.indexOf)d.indexOf=tent.arrays.functions.indexOf;if(!d.lastIndexOf)d.lastIndexOf=
tent.arrays.functions.lastIndexOf;return this};tent.changes.Observable.prototype.removeInterceptor=function(d){var f=this.interceptors[d];if(f._name){if(f.type!=tent.changes.InterceptorTypes.FUNCTION){try{delete this.subject[f.name]}catch(g){tent.log.warn("Error deleting property interceptor '"+f.name+"': "+g)}var i=tent.changes.getPropertyInterceptMode();if(i==tent.changes.PropertyInterceptModes.DEFINEPROPERTY||i==tent.changes.PropertyInterceptModes.DEFINEPROPERTYONLYDOM&&tent.isDOMObject(this.subject))f.descriptor&&
f.descriptor.name&&Object.defineProperty(this.subject,d,f.descriptor);else if(i==tent.changes.PropertyInterceptModes.DEFINESETTER){f.getter&&this.subject.__defineGetter__(d,f.getter);f.setter&&this.subject.__defineSetter__(d,f.setter)}}try{this.subject[f.name]=this.subject[f._name]}catch(k){(i=tent.isDOMObject(this.subject))||tent.log.warn("Error restoring property '"+f.name+"' value: "+k)}try{delete this.subject[f._name]}catch(l){tent.log.warn("Error deleting back storage property '"+f._name+"': "+
...
false;if(this.changesWhileSuspended){if(this.changesWhileSuspended.length<5)for(var d=0,f=this.changesWhileSuspended.length;d<f;d++)this.notifyChange(this.changesWhileSuspended[d]);else this.changesWhileSuspended.length>1&&this.notifyChange(tent.changes.EventTypes.MANYCHANGES,this.changesWhileSuspended);delete this.changesWhileSuspended}};tent.changes.Observable.prototype.isValidHandler=function(d){if(!d)return false;if(typeof d=="function"||typeof d=="object"&&typeof d.handle=="function")return true;
return false};tent.changes.Observable.prototype.bind=function(d,f){if(!f)if(this.isValidHandler(d)){f=d;d=tent.changes.EventTypes.ANY}else throw"must specify a handler: function (Change) or and object with a handle(Change) function";if(!this.isValidHandler(f))throw"must specify a handler: function (Change) or and object with a handle(Change) function";if(!d)d=tent.changes.EventTypes.ANY;var g=this.handlers[d];g||(g=this.handlers[d]=[]);for(var i=0;i<g.length;i++)if(g[i]==f)return false;this.handlers[d].push(f);
return true};tent.changes.Observable.prototype.unbind=function(d,f){if(!f)if(this.isValidHandler(d)){f=d;d=tent.changes.EventTypes.ANY}else throw"must specify a handler to remove";if(!this.isValidHandler(f))throw"must specify a handler to remove";if(!d)d=tent.changes.EventTypes.ANY;var g=this.handlers[d],i=false;if(g)for(var k=0,l=g.length;k<l;k++)if(g[k]==f){g.splice(k,1);k--;l--;i=true}return i};tent.changes.Observable.prototype.unbindWhere=function(d){var f=false,g;for(g in this.handlers)for(var i=
this.handlers[g],k=0,l=i.length;k<l;k++)if(d(i[k])){i.splice(k,1);k--;l--;f=true}return f};tent.changes.Observable.prototype.notifyHandler=function(d,f){try{return typeof f=="function"?f.call(this.subject,d):f.handle(d)}catch(g){var i="";if(g.stack)i+=g.stack;else if(g.message)i+=g.message;tent.log.error("Change Handler Error: "+g+(i?"\n"+i:""));return g}};tent.changes.Observable.prototype.notifyChange=function(d,f){if(this.suspended){if(!this.changesWhileSuspended){this.changesWhileSuspended=[];
...
function(){for(var m=0,n=this.length;m<n;m++)if(this[m].subject==subject&&(this[m].eventType==tent.changes.EventTypes.ADDED||this[m].eventType==tent.changes.EventTypes.REMOVED))return this[m]}}this.changesWhileSuspended.push(new tent.changes.Change(this.subject,d,f))}else{var g;g=d instanceof tent.changes.Change?d:new tent.changes.Change(this.subject,d,f);var i=this.handlers[d];if(i)for(var k=0,l=i.length;k<l;k++)this.notifyHandler(g,i[k]);if(i=this.handlers[tent.changes.EventTypes.ANY]){k=0;for(l=
...
"deepStack"&&opName!="liveHandler")f[opName]=d[opName];d=function(g){if(g.eventType==tent.changes.EventTypes.CHANGED){if(typeof g.data.current=="object")if(f.deepOverDOM||!tent.isDOMObject(g.data.current)){f.deepStack=[g.data.subject];tent.changes.track(g.data.current,f);delete f.deepStack}}else if(g.eventType==tent.changes.EventTypes.ADDED)for(var i=0,k=g.data.items.length;i<k;i++)if(typeof g.data.items[i]=="object")if(f.deepOverDOM||!tent.isDOMObject(g.data.current)){f.deepStack=[g.data.subject];
tent.changes.track(g.data.items[i],f);delete f.deepStack}};d.isLivePropagator=true;return d};tent.changes.track=function(d,f){f||(f={});if(typeof d!="object"){if(!f.deepStack)throw"Cannot track changes of "+typeof d;return false}else if(d==null){if(!f.deepStack)throw"Cannot track changes of null";return false}if(f.attachedObjects===false&&d.__collection__)return false;var g=d instanceof Array,i=false;if(!f.remove&&!f.removeAll&&!d.__observable__){if(f.observable&&(!f.observable.subject||f.observable.subject==
d)){d.__observable__=f.observable;d.__observable__.subject=d}else d.__observable__=new tent.changes.Observable(d);i=true;if(!f.interceptOptions)f.interceptOptions={};g?d.__observable__.interceptArrayModifiers(f.interceptOptions):d.__observable__.interceptProperties(f.interceptOptions)}if(f.parentObject&&typeof f.parentObjectPropertyName){if(d.__observable__.parentObject!==f.parentObject){d.__observable__.parentObject=f.parentObject;i=true}if(d.__observable__.parentObjectPropertyName!==f.parentObjectPropertyName){d.__observable__.parentObjectPropertyName=
f.parentObjectPropertyName;i=true}}else if(f.parentObject===false){if(typeof d.__observable__.parentObject!="undefined"){delete d.__observable__.parentObject;i=true}if(d.__observable__.parentObjectPropertyName!="undefined"){delete d.__observable__.parentObjectPropertyName;i=true}}if(f.bindings)for(var k=0,l=f.bindings.length;k<l;k++){var m=f.bindings[k];if(g&&m.bindArrays!=false||!g&&m.bindObjects!=false)if(f.remove){if(d.__observable__&&d.__observable__.unbind(m.eventType,m.handler))i=true}else if(d.__observable__.bind(m.eventType,
m.handler))i=true}if(f.log)if(d.__observable__&&d.__observable__.log(!f.remove))i=true;if(f.reverseProperties)if(f.remove){if(d.__observable__&&d.__observable__.unbind(tent.changes.reverseProperties.getHandler()))i=true}else if(d.__observable__.bind(tent.changes.reverseProperties.getHandler()))i=true;if(f.removeAll&&d.__observable__){d.__observable__.removeInterceptors();delete d.__observable__.subject;delete d.__observable__;i=true}if(f.deep&&f.live&&!f.remove)if(f.remove){if(d.__observable__.unbindWhere(function(p){return p.isLivePropagator}))i=
true}else{if(!f.liveHandler)f.liveHandler=j(f);if(d.__observable__.bind(f.liveHandler))i=true}if(f.deep){if(f.deepStack)f.deepStack.push(d);else{f.deepStack=[d];if(!f.deepStack.lastIndexOf)f.deepStack.lastIndexOf=tent.arrays.functions.lastIndexOf}if(g){k=0;for(l=d.length;k<l;k++)if((g=d[k])&&typeof g=="object"&&f.deepStack.lastIndexOf(g)<0)if(tent.changes.track(g,f))i=true}else{k=f.propertyFilter||e;for(var n in d)if(typeof d[n]!="function"&&k(d,n)){g=d[n];if(f.deepOverDOM||!tent.isDOMObject(g))if(typeof g==
"object"&&f.deepStack.lastIndexOf(g)<0)if(tent.changes.track(g,f))i=true}}f.deepStack.pop()}return i};tent.changes.untrack=function(d,f){if(f){if(!f.removeAll)f.removeAll=true}else f={removeAll:true};return tent.changes.track(d,f)};tent.changes.isTracked=function(d){return d.__observable__&&d.__observable__ instanceof tent.changes.Observable};tent.changes.bind=function(d,f){var g;if(typeof f=="string"){g=f;f={bindings:[]}}else if(typeof f=="function")f={bindings:[{handler:f}]};else if(typeof f=="object"&&
typeof f.handle=="function")f={bindings:[{handler:f}]};if(arguments.length>2){if(f){if(!f.bindings)f.bindings=[]}else f={bindings:[]};for(var i=2;i<arguments.length;i++)f.bindings.push({eventType:g,handler:arguments[i]})}return tent.changes.track(d,f)};tent.changes.unbind=function(d,f){var g;if(typeof f=="string"){g=f;f={bindings:[]}}else if(typeof f=="function")f={bindings:[{handler:f}]};else if(typeof f=="object"&&typeof f.handle=="function")f={bindings:[{handler:f}]};if(arguments.length>2){if(f){if(!f.bindings)f.bindings=
[]}else f={bindings:[]};for(var i=2;i<arguments.length;i++)f.bindings.push({eventType:g,handler:arguments[i]})}if(f){if(!f.remove)f.remove=true}else f={remove:true};return tent.changes.track(d,f)}});tent.declare("tent.databinding",function(){var a=function(c,e,h,j,d){var f={};f.handle=function(g){var i=false;if(g.eventType==tent.changes.EventTypes.ADDED||g.eventType==tent.changes.EventTypes.REMOVED||g.eventType==tent.changes.EventTypes.CHANGED)if(d.deep)i=true;else{var k=c;if(g.subject==c)i=true;else for(var l=e.split("."),m=0;m<l.length;m++){var n=l[m];k=k[n];if(k==g.subject){i=true;break}}}else if(g.eventType==tent.changes.EventTypes.MANYCHANGES)i=true;if(i){g=c;k=null;if(e){l=e.split(".");
...
g}}};return f};tent.databinding.bind=function(c,e,h,j,d){d||(d={});var f=a(c,e,h,j,d);if(d.deep){if(d.bidirectional)throw"cannot use bidirectional databinding when using a deep tracking";tent.changes.bind(c,{deep:true,live:d.live,log:d.log,bindings:[{handler:f}]})}else{if(d.bidirectional){if(d.template)throw"cannot use bidirectional databinding when using a template";var g=a(h,j,c,e,d);tent.changes.bind(h,{live:d.live,log:d.log,bindings:[{eventType:tent.changes.EventTypes.CHANGED,handler:g}]})}tent.changes.bind(c,
{live:d.live,log:d.log,bindings:[{eventType:tent.changes.EventTypes.CHANGED,handler:f}]})}c=!e?c:c[e];if(d.template)tent.databinding.parseTemplate(d.template,c,h,j);else if(h[j]!=c)h[j]=c};var b={};tent.databinding.parseTemplate=function(c,e,h,j){var d;try{var f=b[c];if(!f){var g="var p=[],print=function (){p.push.apply(p,arguments);};with(obj){p.push('"+c.replace(/[\r\t\n]/g," ").replace(/'(?=[^#]*#>)/g,"\t").split("'").join("\\'").split("\t").join("'").replace(/<#=(.+?)#>/g,"',$1,'").split("<#").join("');").split("#>").join("p.push('")+
"');}return p.join('');";f=new Function("obj",g);b[c]=f}d=f(e)}catch(i){d="< # ERROR: "+i.message+" # >"}if(h)if(j)h[j]=d;else h.innerHTML=d;return d}});tent.declare("tent.changes.reverseProperties",function(){tent.changes.reverseProperties.setReverseProperty=function(b,c,e){e||(e={});e.cardinality=e.cardinality?e.cardinality.toLowerCase():"11";if(!e.reverse)e.reverse=c+"Of";var h={name:e.reverse,cardinality:e.cardinality};if(e.cardinality.substr(0,1)==="n"||e.cardinality.substr(0,1)==="m")h.isArray=true;if(!b.__reverseProperties__)b.__reverseProperties__={};if(e.cardinality.substr(1,1)==="n"||e.cardinality.substr(1,1)==="m"){if(typeof b[c]=="undefined"||
!(b[c]instanceof Array))b[c]=[];b.__reverseProperties__[c]=h;b[c].__parent__=b;b[c].__reverseProperty__=h}else{if(typeof b[c]=="undefined")b[c]=null;b.__reverseProperties__[c]=h}};tent.changes.reverseProperties.setReverseProperties=function(b,c){if(c)for(var e in c)c[e].reverse&&tent.changes.reverseProperties.setReverseProperty(b,e,c[e])};tent.changes.reverseProperties.Set=function(b){this.properties=b};tent.changes.reverseProperties.Set.prototype.create=function(){var b={};this.applyTo(b);return b};
tent.changes.reverseProperties.Set.prototype.applyTo=function(){var b=[];b.bind=function(e){e||(e={});e.reverseProperties=true;for(var h=0;h<this.length;h++){var j=this[h];if(j.__reverseProperties__){tent.changes.bind(j,e);var d=j.__reverseProperties__,f;for(f in d)if(d.hasOwnProperty(f))if(d[f].cardinality&&(d[f].cardinality[1]=="n"||d[f].cardinality[1]=="m")){if(!(j[f]instanceof Array)){j[f]=[];j[f].__parent__=j;j[f].__reverseProperty__=j.__reverseProperties__[f]}tent.changes.bind(j[f],e)}}}};if(this.properties){for(var c=
0;c<arguments.length;c++)tent.changes.reverseProperties.setReverseProperties(arguments[c],this.properties);b.push.apply(b,arguments)}return b};var a;tent.changes.reverseProperties.getHandler=function(){a||(a=function(b){if(b.eventType==tent.changes.EventTypes.ADDED){var c=b.data.items;if(c&&b.subject&&b.subject.__reverseProperty__){var e=b.subject.__reverseProperty__;b=b.subject.__parent__;for(var h=0,j=c.length;h<j;h++){var d=c[h];if(e.isArray){d[e.name]||(d[e.name]=[]);d[e.name].indexOf(b)<0&&d[e.name].push(b)}else if(d[e.name]!=
b){if(typeof d[e.name]!="undefined"&&d[e.name]!=null)throw Error("this."+e.name+" is set. Remove this from its current array before adding it to this array");d[e.name]=b}}}}else if(b.eventType==tent.changes.EventTypes.REMOVED){if((c=b.data.items)&&b.subject&&b.subject.__reverseProperty__){e=b.subject.__reverseProperty__;b=b.subject.__parent__;h=0;for(j=c.length;h<j;h++){d=c[h];if(e.isArray){if(d=d[e.name])for(var f=0,g=d.length;f<g;f++)if(d[f]===b){d.splice(f,1);f--;g--}}else if(d[e.name]===b)d[e.name]=
...
arguments[a];if(b.__entityType__)for(var c in b.__entityType__.properties)if(b[c]!==null&&typeof b[c]=="object"){var e=b.__entityType__.properties[c];if(e.cascadePush){var h=null;if(e.collection)h=this.__collections__[e.collection]||this.createCollection(e.collection);if(b[c]instanceof Array){if(e.cardinality&&e.cardinality[1]!="1")h?h.push.apply(h,b[c]):this.push.apply(this,b[c])}else h?h.push(b[c]):this.push(b[c])}}}};tent.entities.Context.prototype.__cascadeAttach__=function(){for(var a=0;a<arguments.length;a++){var b=
arguments[a];if(b.__entityType__)for(var c in b.__entityType__.properties)if(b[c]!==null&&typeof b[c]=="object"){var e=b.__entityType__.properties[c];if(e.cascadeAttach){var h=null;if(e.collection)h=this.__collections__[e.collection]||this.createCollection(e.collection);if(b[c]instanceof Array){if(e.cardinality&&e.cardinality[1]!="1")h?h.attach.apply(h,b[c]):this.attach.apply(this,b[c])}else h?h.attach(b[c]):this.attach(b[c])}}}};tent.entities.Context.prototype.__cascadeRemove__=function(){for(var a=
...
function(a){var b=this.context.changes;a.__changeState__!=tent.entities.ChangeStates.UNCHANGED&&b&&b.items.remove(a);delete a.__changeState__;tent.changes.unbind(a,this);this.unbindArrayProperties(a)};tent.entities.ContextChangeHandler.prototype.objectLinked=function(a,b,c,e){if(typeof e=="object")if(e instanceof Array){if(e.__parent__!=a){if(e.__parent__)throw"this array already has another parent object";e.__parent__=a}if(e.__propertyName__!=b)e.__propertyName__=b;tent.changes.bind(e,this);c.onLinkPush&&
this.context.attach.push(this.context,e)}else if(e!==null&&typeof e!="undefined"){if(e.__changeState__==tent.entities.ChangeStates.DELETED)throw'cannot link a DELETED object to property "'+c+'"';if(e.__collection__){if(e.__collection__.context!=this.context)throw"cannot link this object, it belongs to another Context";if(c.collection&&e.__collection__.name!=c.collection)throw"cannot link an object of collection "+c.collection+' to property "'+c+'"';}if(!e.__collection__&&c.onLinkPush){e.__entityType__&&
this.context.push(e.__entityType__);if(c.collection){if(!this.context.__collections__[c.collection])throw Error('Collection "'+c.collection+'" not found in this Context');this.context.__collections__[c.collection].push(e)}else this.context.push(e)}if(c.trackLinks){c=null;for(var h=this.context.changes.items.length-1;h>=0;h--){var j=this.context.changes.items[h];if(j instanceof tent.entities.EntityLink&&j.from===a&&j.to===e&&j.propertyName==b){c=j;break}}if(c){if(c.__changeState__==tent.entities.ChangeStates.DELETED)c.__changeState__=
tent.entities.ChangeStates.UNCHANGED}else{c=new tent.entities.EntityLink(a,e,b,tent.entities.ChangeStates.ADDED);this.context.changes.items.push(c)}}}};tent.entities.ContextChangeHandler.prototype.objectUnlinked=function(a,b,c,e){if(typeof e=="object")if(e instanceof Array){delete e.__parent__;delete e.__propertyName__;tent.changes.unbind(e,this);c.onUnlinkRemove&&this.context.remove.apply(this.context,e)}else if(e!==null&&typeof e!="undefined"){c.onUnlinkRemove&&e.__collection__&&e.__collection__.remove(e);
if(c.trackLinks){c=null;for(var h=-1,j=this.context.changes.items.length-1;j>=0;j--){var d=this.context.changes.items[j];if(d instanceof tent.entities.EntityLink&&d.from===a&&d.to===e&&d.propertyName==b){c=d;h=j;break}}if(c)if(c.__changeState__==tent.entities.ChangeStates.ADDED)this.context.changes.items.splice(h,1);else{if(c.__changeState__!=tent.entities.ChangeStates.DELETED)c.__changeState__=tent.entities.ChangeStates.DELETED}else{c=new tent.entities.EntityLink(a,e,b,tent.entities.ChangeStates.DELETED);
...
this.getTrackedProperty(a.subject,a.data.propertyName)){this.context.touch(a.subject);this.objectUnlinked(a.subject,a.data.propertyName,b,a.data.oldValue);this.objectLinked(a.subject,a.data.propertyName,b,a.data.current)}}else this.context.touch(a.subject);else if(a.eventType==tent.changes.EventTypes.ADDED){var c=a.subject.__parent__;if(c&&(b=this.getTrackedProperty(c,a.data.propertyName)))for(var e=0,h=a.data.items.length;e<h;e++)this.objectLinked(c,a.data.propertyName,b,a.data.items[e])}else if(a.eventType==
tent.changes.EventTypes.REMOVED)if((c=a.subject.__parent__)&&(b=this.getTrackedProperty(c,a.data.propertyName))){e=0;for(h=a.data.items.length;e<h;e++)this.objectUnlinked(c,a.data.propertyName,b,a.data.items[e])}tent.changes.reverseProperties.getHandler()(a)}};tent.entities.ContextChangeHandler.prototype.init=function(){if(this.context.log)var a=new tent.coreTypes.NameCounter;for(var b in this.context.__collections__){for(var c=this.context.__collections__[b],e=0,h=c.items.length;e<h;e++)this.itemAttached(c.items[e]);
...
tent.entities.ContextChangeHandler.prototype.acceptChanges=function(){for(var a=0,b=arguments.length;a<b;a++){var c=arguments[a],e;if(typeof c=="number"){e=c;if(e<0||this.context.changes.items.length<e-1){c=null;e=-1}else c=this.context.changes.items[e]}else e=this.context.changes.items.lastIndexOf(c);if(e>=0&&c){this.context.changes.items.splice(e,1);if(c.__changeState__==tent.entities.ChangeStates.ADDED)c.__changeState__=tent.entities.ChangeStates.UNCHANGED;else if(c.__changeState__==tent.entities.ChangeStates.MODIFIED)c.__changeState__=
tent.entities.ChangeStates.UNCHANGED;else if(c.__changeState__==tent.entities.ChangeStates.DELETED)c.__changeState__=tent.entities.ChangeStates.DETACHED}}}});tent.declare("tent.persisters.rest",function(){tent.persisters.rest.uriCombine=function(){for(var a="",b=0,c=arguments.length;b<c;b++){var e=arguments[b]+"";if(e)if(e.match(/^[A-Za-z]+\:\/\//))a=e;else a+=a.substr(a.length-1,1)=="/"?e.substr(0,1)=="/"?e.substr(1):e.substr(0,2)=="./"?e.substr(2):e:e.substr(0,1)=="/"?e:e.substr(0,2)=="./"?e.substr(1):"/"+e}return a};tent.persisters.rest.createItemMapper=function(a){a||(a={});return function(b,c,e){if(b=b instanceof Array?b:c.multi?(c.multiSelector||
...
c.itemTransformer||a.itemTransformer;if(f)d=f(d,c)}d&&e(d,c)}}};tent.persisters.rest.createChangeSerializer=function(a){a||(a={});return function(b,c){var e=c.itemSerializer||a.itemSerializer||function(g){if(g instanceof tent.entities.EntityLink)return null;var i=tent.clone(g,{deep:true,onlyOwnProperties:true,attachedObjects:false,skipPrivates:true});if(g._id)i._id=tent.pget(g,"_id");if(g._rev)i._rev=tent.pget(g,"_rev");if(g.__changeState__===tent.entities.ChangeStates.DELETED)i._deleted=true;return i};
if(b instanceof Array){for(var h=[],j=0,d=b.length;j<d;j++)h.push(e(b[j],c));var f=c.saveWrapperProperty||a.saveWrapperProperty||"docs";return(c.itemsSaveWrapper||a.itemsSaveWrapper||function(g){var i={};i[f]=g;return i})(h,c)}else if(typeof b=="object")return e(b,c)}};tent.persisters.rest.createChangeResponseMapper=function(a){a||(a={});return function(b,c,e){var h;if(h=b instanceof Array?b:(c.resultSelector||a.resultSelector||function(g,i){var k=i.resultSelectorProperty||a.resultSelectorProperty;
if(k)return g[k];else{h=g;for(var l in g)if(g[l]instanceof Array)return g[l]}})(b,c)){b=0;for(var j=h.length;b<j;b++){var d=h[b];if(d=(c.resultItemSelector||a.resultItemSelector||function(g,i){var k=i.resultItemSelectorProperty||a.resultItemSelectorProperty;return k?g[k]:g})(d,c)){var f=c.resultItemTransformer||a.resultItemTransformer;if(f)d=f(d,c)}if(d){f=(c.localChangeFinder||a.localChangeFinder||function(g,i){if(g.hasChanges())for(var k=0,l=g.changes.items.length;k<l;k++){if(typeof i.id!="undefined"&&
g.changes.items[k]._id===i.id)return k;if(typeof i.id!="undefined"&&g.changes.items[k].id===i.id)return k;if(typeof i._id!="undefined"&&g.changes.items[k]._id===i._id)return k;if(typeof i._id!="undefined"&&g.changes.items[k].id===i._id)return k}})(c.context,d,c);typeof f=="number"&&f>=0||c.unorderedResults||a.unorderedResults||(f=0);typeof f=="number"&&f>=0&&e(d,f,c)}}}}};tent.persisters.rest.RestPersister=function(){this.baseUri="http://127.0.0.1:5984/mydb";this.saving=false;this.savingErrors=[];
this.loadingErrors=[];this.loadItemMapper=null;this.localItemFinder=function(a,b){return a.filter(function(c){if(typeof b._id!="undefined"&&c._id===b._id||typeof b.id!="undefined"&&c.id===b.id)return true;return false})[0]};this.versionEquals=function(a,b){return a._rev===b._rev};this.updateLocalVersion=function(a,b){a._rev=b.rev||b._rev;if(typeof b.id!="undefined"&&b.id!==a._id)a._id=b.id};this.isDeleted=function(a){return a._deleted};this.bulkSaveUri=this.changeResponseMapper=this.changeSerializer=
null};tent.persisters.rest.RestPersister.prototype.load=function(a,b,c){try{if(!a)throw"a context is required for loading entities";if(!b)throw"an url must be specified to load";if(this.loading)throw"already loading entities";this.loading=true;var e=this;c||(c={});var h=c.complete;c.complete=function(d){e.loading=false;d.error&&e.loadingErrors.push(d.error);h&&h(d)};this.__load__(a,b,c)}catch(j){this.loadingErrors.push(j);this.loading=false;c.complete&&c.complete({error:j})}};tent.persisters.rest.RestPersister.prototype.__load__=
function(a,b,c){if(typeof jQuery=="undefined"||typeof jQuery.ajax=="undefined")throw"jQuery.ajax is required in order to load entities";b=tent.persisters.rest.uriCombine(this.baseUri,b);jQuery.ajax({context:{persister:this,context:a,options:c},url:b,beforeSend:function(e){if(c.credentials){e.setRequestHeader("Origin",document.location.protocol+"//"+document.location.host);e.setRequestHeader("Authorization","Basic "+c.credentials)}},type:c.method||"GET",cache:!!c.cache,dataType:"json",success:function(e){var h=
{persister:this,options:c};try{this.persister.__processLoadResponse__(this.context,e,c,h);h.ok=true}catch(j){h.error=j}c.complete&&c.complete(h)},error:function(e,h,j){e={persister:this,options:c,error:{type:h,errorThrown:j}};c.complete&&c.complete(e)}})};tent.persisters.rest.RestPersister.prototype.__processLoadResponse__=function(a,b,c){if(c.context!==a)c.context=a;if(!this.loadItemMapper)this.loadItemMapper=tent.persisters.rest.createItemMapper();var e=this;this.loadItemMapper(b,c,function(h){var j=
e.localItemFinder(a,h);if(j)if(j.__changeState__===tent.entities.ChangeStates.MODIFIED||j.__changeState__===tent.entities.ChangeStates.DELETED)e.versionEquals(j,h)||j.__loadErrors__.push({error:"conflict",reason:"document modified recently by another user"});else{if(e.isDeleted(h)){a.remove(j);a.detach(j)}else{tent.pset(j,h,true);a.changeHandler&&a.changeHandler.trackComplexProperties(j)}j.__loadErrors__&&delete j.__loadErrors__}else a.attach(h)})};tent.persisters.rest.RestPersister.prototype.saveChanges=
function(a,b){try{if(!a)throw"a context is required for saving changes";b||(b={});if(!b.context)b.context=a;if(a.hasChanges()){if(this.saving)throw"Already saving changes";this.saving=true;var c=this,e=b.complete;b.complete=function(j){c.saving=false;if(j.error)c.savingErrors.push(j.error);else if(typeof j.ok=="undefined")j.ok=true;e&&e(j)};this.__persist__(a,b)}else b.complete&&b.complete({ok:true,nochanges:true})}catch(h){this.savingErrors.push(h);this.saving=false;b.complete&&b.complete({error:h})}};
tent.persisters.rest.RestPersister.prototype.__persist__=function(a,b){if(typeof jQuery=="undefined"||typeof jQuery.ajax=="undefined")throw"jQuery.ajax is required in order to persist changes";if(this.bulkSaveUri){var c=tent.persisters.rest.uriCombine(this.baseUri,this.bulkSaveUri),e=this.__getPersistData__(a,b);if(e==null){c={persister:this,ok:true,nochanges:true};b.complete&&b.complete(c)}else jQuery.ajax({context:{persister:this,context:a},url:c,beforeSend:function(h){if(b.credentials){h.setRequestHeader("Origin",
document.location.protocol+"//"+document.location.host);h.setRequestHeader("Authorization","Basic "+b.credentials)}},type:"POST",cache:false,data:JSON.stringify(e),dataType:"json",contentType:"application/json",success:function(h){var j={persister:this};try{this.persister.__processPersistResponse__(this.context,h,b,j);j.ok=true}catch(d){j.error=d}b.complete&&b.complete(j)},error:function(h,j,d){h={persister:this,error:{type:j,errorThrown:d}};b.complete&&b.complete(h)}})}else throw"individual change save not implemented";
};tent.persisters.rest.RestPersister.prototype.__getPersistData__=function(a,b){if(!this.changeSerializer)this.changeSerializer=tent.persisters.rest.createChangeSerializer();var c=null;if(a.hasChanges())c=this.changeSerializer(a.changes.items,b);return c};tent.persisters.rest.RestPersister.prototype.__processPersistResponse__=function(a,b,c){if(a.hasChanges()){if(!this.changeResponseMapper)this.changeResponseMapper=tent.persisters.rest.createChangeResponseMapper();var e=this;this.changeResponseMapper(b,
c,function(h,j){var d=a.changes.items[j];if(h.error){if(!d.__saveErrors__)d.__saveErrors__=[];d.__saveErrors__.push({error:h.error,reason:h.reason})}else{e.updateLocalVersion(d,h);d.__saveErrors__&&delete d.__saveErrors__;a.acceptChanges(j)}})}}});tent.declare("tent.persisters.couchdb",function(){tent.persisters.couchdb.CouchDBPersister=function(){tent.persisters.rest.RestPersister.apply(this,arguments);this.bulkSaveUri="_bulk_docs";this.loadItemMapper=tent.persisters.rest.createItemMapper({multiSelectorProperty:"rows",multiItemSelectorProperty:"doc"})};tent.mixin(tent.persisters.couchdb.CouchDBPersister,tent.persisters.rest.RestPersister)});
